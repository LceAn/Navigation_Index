...
| 项目 | 说明 |
|------|------|
| **图片事件监听** | 修复 load/error 事件不冒泡导致卡片动画失效，改为直接绑定每张图片 |
| **CSS 优先级** | `#box .card` 显式重写 transition，防止被 `.card` 覆盖 |
| **Loader 可见性** | 兜底 timeout 从 8s 缩短为 1s，配合 3 秒最小显示时间 |
| **Stagger 间隔** | 从 60ms 调整为 80ms，弹出效果更明显 |
...
| 项目 | 说明 |
|------|------|
| **Stagger 动画** | 卡片依次弹出，每张间隔 60ms，形成波浪效果 |
| **弹性缓动** | cubic-bezier(0.16,1,0.3,1) 弹性曲线 |
| **组合动画** | 淡入 + 上浮 30px + 缩放 0.95→1 |
...
| 项目 | 说明 |
|------|------|
| **月牙效果** | 月亮改为月牙形（inset box-shadow 遮挡），支持随机大小 80~140px |
| **月牙方向** | 内阴影 x 轴反转，光晕自适应 calc() 跟随月亮大小 |
| **"更多"页面重构** | 完全重写 more.html，CSS 变量适配夜间模式，现代化卡片设计 |
| **天空背景修复** | `#product` 白色背景覆盖 fixed 天空，子页面加载无横条 |
| **滚动体验** | 删除 updateUI 中的天空隐藏逻辑，滚动时欢迎界面不再变灰 |
| **loadPageContent** | 移入 DOMContentLoaded 内部，修复作用域问题 |
